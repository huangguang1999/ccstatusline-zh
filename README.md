...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读写组件、已用超额用量组件、压缩计数触发分类与已回收 Token、字形符号自定义、变暗样式、配置恢复徽章等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **77 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.22 新增的缓存命中率 / 缓存读取 / 缓存写入 / 已用超额用量）
...
- **缓存命中率 / 缓存读取 / 缓存写入**新组件（单轮/会话作用域切换、空时隐藏）、**已用超额用量**新组件、**压缩计数触发分类与已回收 Token 显示**、**字形符号自定义**（(g)字形 快捷键）、**变暗样式**（(d)变暗）、**配置加载失败恢复徽章**、**Powerline 弹性分隔符支持**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存组件 / 已用超额用量 / 压缩计数增强 / 字形自定义 / 变暗样式 / 中文化覆盖） |
...
| 组件       | 说明                                      |
| ---------- | ----------------------------------------- |
| 缓存命中率 | 显示 Token 缓存命中率（支持单轮/会话切换） |
| 缓存读取   | 显示从缓存读取的 Token 数量及上下文占比   |
| 缓存写入   | 显示写入缓存的 Token 数量及上下文占比     |
