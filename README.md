...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率 / 缓存读写组件、超额已用量组件、组件维度暗色样式、符号字形覆盖、配置错误恢复提示等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **77 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.21–v2.2.22 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超额已用量）
...
- **缓存命中率 / 缓存读取 / 缓存写入**（提示缓存 Token 维度，支持按次/会话切换）、**超额已用量组件**（按需付费超额消费）、**组件维度暗色样式**（[加粗] / [暗色] / [括号暗色]）、**符号字形覆盖**（GitBranch / GitWorktree / CompactionCounter 等，按 g 键进入）、**配置错误恢复提示**（⚠ 配置无效 badge）、**上下文进度条 1M+ 显示修复**（≥1M 显示为 1.0M）、**CompactionCounter 触发来源 / 回收 Token 计数**、**ThinkingEffort Ultracode 模式支持**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率/读取/写入组件 / 超额已用量 / 暗色样式 / 符号字形覆盖 / 配置恢复提示 / 中文化覆盖） |
...
| 组件       | 说明                                          |
| ---------- | --------------------------------------------- |
| 缓存命中率 | 显示提示缓存命中率（缓存读取 vs 写入），支持按次/会话切换 |
| 缓存读取   | 显示从缓存读取的 Token 数及上下文占比，支持按次/会话切换 |
| 缓存写入   | 显示写入缓存的 Token 数及上下文占比，支持按次/会话切换 |
