...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含字形符号自定义覆盖、缓存命中率/读取/写入组件、超用量已用组件、压缩计数触发分组与回收 Token 展示、Powerline 弹性分隔符、每组件暗淡样式、`--version` 参数等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **83 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.21 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超用量已用，v2.2.21–v2.2.22 新增的压缩计数增强 / 字形符号覆盖）
...
- **字形符号自定义覆盖**（每组件可替换默认符号/Emoji）、**缓存命中率 / 缓存读取 / 缓存写入组件**、**超用量已用组件**、**压缩计数触发分组与回收 Token 展示**、**每组件暗淡（dim）样式**、**自定义命令 `terminal_width` 变量**、**配置非破坏性恢复与符号链接保留**、**ContextBar 1.0M 格式修正**（v2.2.21）
- **压缩计数回收 Token 字形覆盖**、**Powerline 模式弹性分隔符支持**、**`--version` 命令行参数**（v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 字形符号覆盖 / 缓存命中率·读取·写入 / 超用量已用 / Powerline 弹性分隔符 / --version / 中文化覆盖） |
...
| 组件       | 说明                                             |
| ---------- | ------------------------------------------------ |
| 缓存命中率 | 显示 Token 缓存命中率（缓存读取/（缓存读取+输入））|
| 缓存读取   | 显示缓存读取 Token 数量                          |
| 缓存写入   | 显示缓存写入 Token 数量                          |
