...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读取/写入组件、超额使用花费组件、组件符号自定义、压缩计数按触发方式拆分与已回收 token 数、每组件变暗样式、Powerline 模式下弹性分隔符修复、配置文件非破坏性恢复等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **77 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.21 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超额使用花费）
...
- **组件符号自定义**（Git / Jujutsu 系列组件的图标可自由替换）、**缓存命中率 / 缓存读取 / 缓存写入组件**、**超额使用花费组件**、**Custom Command stdin 附带终端宽度**、**压缩计数按触发方式拆分与已回收 token 数**、**超额用量货币按 API 返回格式化**、**Token 数四舍五入至 1M 显示修正**（v2.2.21）
- **每组件变暗样式**（整体变暗 / 仅括号变暗）、**配置文件非破坏性恢复**（解析失败时不再覆盖用户文件，状态栏显示醒目的"配置无效"提示）、**settings.json 原子写入与软链接保留**、**上下文进度条 1M 显示修正**、**压缩计数回收符号可自定义**、**Powerline 模式下弹性分隔符生效修复**（v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率/读取/写入 / 超额使用花费 / 组件符号自定义 / 每组件变暗样式 / 配置文件非破坏性恢复 / Powerline 弹性分隔符修复 / 中文化覆盖） |
...
| 组件         | 说明                                   |
| ------------ | -------------------------------------- |
| 缓存命中率   | 显示提示缓存命中率（缓存读取 vs 缓存写入） |
| 缓存读取     | 显示从缓存中读取的 token 数及其上下文占比 |
| 缓存写入     | 显示写入缓存的 token 数及其上下文占比   |
