...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读取/写入组件、压缩计数可回收 token 箭头字形覆盖、Powerline 弹性分隔符修复、超额用量已用组件、暗淡样式组件支持等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **77 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.21 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超额用量已用）
...
- **缓存命中率 / 缓存读取 / 缓存写入 / 超额用量已用**四个新组件、**per-widget 暗淡样式**（`dim: true` / `dim: 'parens'`）、**颜色菜单暗淡快捷键 `(d)`**、**CompactionCounter 可回收 token 箭头字形覆盖**、**formatTokens 精确 1M 边界修正**、**Powerline 弹性分隔符渲染修复**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ Cache 组件 / per-widget 暗淡样式 / CompactionCounter 字形覆盖 / Powerline 弹性分隔符修复 / 中文化覆盖） |

---

## ✨ 功能特性

- **77 种可定制组件** — 模型、Git（含 PR / 冲突 / 暂存 / Origin / Upstream / 工作树等细分组件）、Token、缓存、上下文、会话、费用、速度、超额用量等
...
| 组件       | 说明                                           |
| ---------- | ---------------------------------------------- |
| 缓存命中率 | 显示提示词缓存命中率（缓存读取 vs 缓存写入）   |
| 缓存读取   | 显示从缓存命中的 Token 数及其占上下文的比例    |
| 缓存写入   | 显示写入缓存的 Token 数及其占上下文的比例      |
...
| 超额用量已用   | 显示超额用量（按量付费超出部分）的已用金额（美元） |
