...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读写组件、超额用量已用组件、压缩计数触发分组与已回收 Token、颜色菜单暗色样式指示器等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **缓存命中率 / 缓存读取 / 缓存写入 / 超额用量已用**四个新组件、**压缩计数触发分组与已回收 Token 显示**、**GitStatus / GitAheadBehind 符号自定义槽位**、**Hook 输入处理重构**（v2.2.21）
- **颜色菜单暗色样式指示器**（加粗 / 暗色 / 括号暗色三态）、**Powerline 模式弹性分隔符提示优化**、**ContextBar Token 显示精度修正**（v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率 / 缓存读写 / 超额已用 / 压缩计数增强 / 暗色样式指示器 / 中文化覆盖） |
...
| 缓存读取     | 显示当前会话缓存读取 Token 数量              |
| 缓存写入     | 显示当前会话缓存写入 Token 数量              |
...
| 压缩计数         | 显示当前会话上下文压缩次数，可选触发分组细分和已回收 Token 显示 |
