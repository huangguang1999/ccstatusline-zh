...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读取/写入组件、超额用量已用、压缩计数器增强、字形覆盖、颜色暗淡样式等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **缓存命中率 / 缓存读取 / 缓存写入组件**（按轮次/会话切换、空时隐藏）、**超额用量已用组件**、**压缩计数器按触发类型拆分 + tokens 回收量显示**、**字形覆盖（GitBranch/GitWorktree/CompactionCounter 等）**、**颜色菜单单组件 dim 暗淡样式**、**上下文进度条 1.0M 格式**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率/读取/写入 / 超额用量已用 / 压缩计数器增强 / 字形覆盖 / dim 样式 / 中文化覆盖） |

---

## ✨ 功能特性

- **77 种可定制组件** — 模型、Git（含 PR / 冲突 / 暂存 / Origin / Upstream / 工作树等细分组件）、Token、上下文、缓存、会话、费用、速度等
...
| 组件       | 说明                                            |
| ---------- | ----------------------------------------------- |
| 缓存命中率 | 显示提示缓存命中率（缓存读取 vs 缓存写入），支持按轮次/会话切换 |
| 缓存读取   | 显示缓存读取 token 数量，支持按轮次/会话切换    |
| 缓存写入   | 显示缓存写入 token 数量，支持按轮次/会话切换    |
