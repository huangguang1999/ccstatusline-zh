...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率 / 缓存读写、超额已用量、按组件 Dim 样式、Flex 分隔符 Powerline 修复等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **77 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.21 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超额已用量）
...
- **缓存命中率 / 缓存读取 / 缓存写入组件**（支持按轮次 / 会话切换、空时隐藏）、**超额已用量组件**、**符号字形覆盖**（GitBranch / JjBookmarks / JjWorkspace / CompactionCounter 均可自定义前缀字形）、**压缩计数器触发分割与已回收 Token 支持**、**按组件 Dim 减淡样式**、**设置文件非破坏性恢复与无效 JSON 警告**、**弹性分隔符 Powerline 路径修复**、**上下文进度条 ≥1M token 显示修正**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率 / 缓存读写 / 超额已用量 / 符号字形覆盖 / 按组件 Dim 样式 / 弹性分隔符 Powerline 修复 / 中文化覆盖） |
