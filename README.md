...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读写组件、压缩计数触发拆分、已用超额额度、每组件暗淡样式、可覆盖字形符号、弹性分隔符 Powerline 修复等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **缓存命中率 / 缓存读取 / 缓存写入**（3 个新组件）、**已用超额额度**组件、**压缩计数触发拆分与已回收 Token 显示**、**每组件暗淡样式**（整体暗淡或仅括号内暗淡）、**可覆盖字形符号**（部分组件支持自定义符号字形）、**弹性分隔符 Powerline 渲染修复**、**上下文条 ≥1M Token 显示修复**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率/读写 / 已用超额 / 压缩触发拆分 / 字形符号覆盖 / 每组件暗淡 / 中文化覆盖） |
