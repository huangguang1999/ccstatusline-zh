...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率组件、超额用量已使用组件、符号覆盖、暗色（dim）样式、per-widget 符号自定义、配置文件安全恢复等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **83 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.22 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超额用量已使用）
...
- **缓存命中率 / 缓存读取 / 缓存写入组件**、**超额用量已使用组件**、**per-widget 符号自定义覆盖**、**暗色（dim）样式支持**、**配置文件安全恢复与无效 JSON 警告**、**符号链接 settings.json 保护**、**Context Bar 百万 token 显示修正**、**Compaction Counter 转录标记计数**（v2.2.21）
- **Flex separator 在 Powerline 模式下正常工作**、**renderer 999950-999999 Token 显示修正**、**测试套件 Windows 兼容**（v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存命中率 / 超额用量已使用 / 符号覆盖 / 暗色样式 / 配置恢复 / 中文化覆盖） |
...
| 组件       | 说明                                   |
| ---------- | -------------------------------------- |
| 缓存命中率 | 显示提示缓存命中率（缓存读取 vs 写入） |
| 缓存读取   | 显示从缓存中读取的 Token 数及占比      |
| 缓存写入   | 显示写入缓存的 Token 数及占比          |
