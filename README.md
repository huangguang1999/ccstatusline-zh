...
> 本项目是 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的**中文汉化 Fork**，当前同步至上游 v2.2.22 版本（含缓存命中率/读取/写入组件、超额用量（已用）组件、暗色样式、字符符号覆盖、Powerline 弹性分隔符等最新功能）。所有用户可见的界面文本（组件名称、分类、描述、菜单标签、提示信息等）均已翻译为中文，方便中文用户使用。
...
- **77 个组件**的名称、描述、分类标签（含 v2.2.13 新增的 Voice Status / 周 Sonnet 用量 / 周 Opus 用量，v2.2.17 新增的超额用量占比 / 超额用量剩余，v2.2.20 新增的 Remote Control Status，v2.2.21–v2.2.22 新增的缓存命中率 / 缓存读取 / 缓存写入 / 超额用量已用）
...
- **暗色样式（dim）支持**、**缓存命中率 / 缓存读取 / 缓存写入组件**（API 缓存指标）、**超额用量（已用）组件**、**压缩计数器已回收 Token 箭头字符符号可覆盖**、**Powerline 模式支持弹性分隔符**、**上下文条 1M 格式修正**（v2.2.21–v2.2.22）
- **确认对话框** "是 / 否"
- **分类筛选** "全部" 等界面元素
...
| 功能差异   | —            | 无，功能完全一致          |
| 同步版本   | 最新         | v2.2.22（+ 缓存组件 / 超额用量已用 / 暗色样式 / 字符符号覆盖 / Powerline 弹性分隔符 / 中文化覆盖） |
...
| 组件       | 说明                              |
| ---------- | --------------------------------- |
| 缓存命中率 | 显示 API 缓存命中率（本轮/会话） |
| 缓存读取   | 显示 API 缓存读取 Token 数        |
| 缓存写入   | 显示 API 缓存写入 Token 数        |
