...
export class CustomCommandWidget implements Widget {
    ...
    getDescription(): string { return '执行自定义 Shell 命令并显示输出'; }
    getDisplayName(): string { return '自定义命令'; }
    ...
    getCustomKeybinds(): CustomKeybind[] {
        return [
            { key: 'e', label: '(e)编辑命令', action: 'edit-command' },
            { key: 'w', label: '(w)最大宽度', action: 'edit-width' },
            { key: 't', label: '(t)超时时间', action: 'edit-timeout' },
            { key: 'p', label: '(p)保留颜色', action: 'toggle-preserve' }
        ];
    }
