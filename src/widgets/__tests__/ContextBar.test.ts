...
describe('ContextBarWidget', () => {
    ...
    it('renders from context_window data when available', () => {
        ...
        expect(widget.render({ id: 'ctx', type: 'context-bar' }, context, DEFAULT_SETTINGS)).toBe('上下文: [bar:15.0:16] 30k/200k (15%)');
    });
    ...
    it('falls back to token metrics and model context size', () => {
        ...
        expect(widget.render({ id: 'ctx', type: 'context-bar' }, context, DEFAULT_SETTINGS)).toBe('上下文: [bar:25.0:16] 50k/200k (25%)');
    });
    ...
    it('uses 1M context label model IDs in fallback mode', () => {
        ...
        expect(widget.render({ id: 'ctx', type: 'context-bar' }, context, DEFAULT_SETTINGS)).toBe('上下文: [bar:5.0:16] 50k/1000k (5%)');
    });
    ...
    it('clamps usage percentage to 100 when context length exceeds total', () => {
        ...
        expect(widget.render({ id: 'ctx', type: 'context-bar' }, context, DEFAULT_SETTINGS)).toBe('上下文: [bar:100.0:16] 250k/200k (100%)');
    });
    ...
    it('renders long progress bar mode when configured', () => {
        ...
        }, context, DEFAULT_SETTINGS)).toBe('上下文: [bar:15.0:32] 30k/200k (15%)');
    });
