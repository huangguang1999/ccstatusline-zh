...
describe('FreeMemoryWidget', () => {
    ...
    describe('metadata', () => {
        it('should return correct display name', () => {
            expect(widget.getDisplayName()).toBe('内存用量');
        });

        it('should return correct description', () => {
            expect(widget.getDescription()).toBe('显示系统内存使用情况（已用/总量）');
        });
    ...
    describe('preview mode', () => {
        it('should return mock data with label in preview mode', () => {
            ...
            expect(result).toBe('内存: 12.4G/16.0G');
        });
...
`);
            ...
            expect(result).toBe('内存: 9.2G/16.0G');
        });
...
`);
    ...
    describe('render on non-macOS (os.freemem fallback)', () => {
        ...
        it('should use total - free calculation on Linux', () => {
            ...
            expect(result).toBe('内存: 8.0G/16.0G');
        });
