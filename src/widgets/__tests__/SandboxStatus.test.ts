...
describe('SandboxStatusWidget', () => {
    describe('metadata', () => {
        it('has correct display name', () => {
            expect(new SandboxStatusWidget().getDisplayName()).toBe('沙箱状态');
        });
        ...
        it('has correct category', () => {
            expect(new SandboxStatusWidget().getCategory()).toBe('核心');
        });
    ...
    describe('render() - format text', () => {
        ...
        it('renders SB: OFF when OFF', () => {
            vi.spyOn(claudeSettings, 'getSandboxConfig').mockReturnValue({ enabled: false });
            expect(new SandboxStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('SB: 关闭');
        });

        it('renders SB: ON when ON', () => {
            vi.spyOn(claudeSettings, 'getSandboxConfig').mockReturnValue({ enabled: true });
            expect(new SandboxStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('SB: 启用');
        });
    });
    ...
    describe('render() - format word', () => {
        ...
        it('renders Sandbox: OFF when OFF', () => {
            vi.spyOn(claudeSettings, 'getSandboxConfig').mockReturnValue({ enabled: false });
            expect(new SandboxStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('沙箱: 关闭');
        });

        it('renders Sandbox: ON when ON', () => {
            vi.spyOn(claudeSettings, 'getSandboxConfig').mockReturnValue({ enabled: true });
            expect(new SandboxStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('沙箱: 启用');
        });
    });
