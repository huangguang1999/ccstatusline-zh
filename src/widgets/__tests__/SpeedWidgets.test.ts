...
describe('OutputSpeedWidget', () => {
    ...
    it('should describe session-average behavior and window override', () => {
        expect(widget.getDescription()).toContain('全会话平均');
        expect(widget.getDescription()).toContain('0-120');
    });

    it('should expose a window editor keybind', () => {
        expect(widget.getCustomKeybinds()).toEqual([
            { key: 'w', label: '(w)时间窗口', action: 'edit-window' }
        ]);
    });

    it('should show session average as the default editor modifier', () => {
        expect(widget.getEditorDisplay(createItem('output-speed')).modifierText).toBe('(全会话平均)');
    });

    it('should render session preview value by default', () => {
        const context: RenderContext = { isPreview: true };
        expect(widget.render(createItem('output-speed'), context, DEFAULT_SETTINGS)).toBe('输出: 42.5 t/s');
    });
    ...
    it('should render window preview when window metadata is enabled', () => {
        ...
        expect(widget.render(item, context, DEFAULT_SETTINGS)).toBe('输出: 26.8 t/s');
    });
    ...
    it('should calculate output speed from session speedMetrics when window is disabled', () => {
        ...
        expect(widget.render(createItem('output-speed'), context, DEFAULT_SETTINGS)).toBe('输出: 50.0 t/s');
    });
    ...
    it('should calculate output speed from windowed metrics when window is enabled', () => {
        ...
        expect(widget.render(item, context, DEFAULT_SETTINGS)).toBe('输出: 120.0 t/s');
    });
});
...
describe('InputSpeedWidget', () => {
    ...
    it('should show configured window modifier in editor display', () => {
        ...
        expect(widget.getEditorDisplay(item).modifierText).toBe('(75秒窗口)');
    });

    it('should render session preview by default and raw variant', () => {
        const context: RenderContext = { isPreview: true };
        expect(widget.render(createItem('input-speed'), context, DEFAULT_SETTINGS)).toBe('输入: 85.2 t/s');
    ...
    it('should use windowed speed metrics for render output when configured', () => {
        ...
        expect(widget.render(item, context, DEFAULT_SETTINGS)).toBe('输入: 150.0 t/s');
    });
    ...
    it('should treat 0 as disabled window and use session metrics', () => {
        ...
        expect(widget.render(item, context, DEFAULT_SETTINGS)).toBe('输入: 100.0 t/s');
    });
});
...
describe('TotalSpeedWidget', () => {
    ...
    it('should clamp invalid editor metadata to supported range', () => {
        ...
        expect(widget.getEditorDisplay(item).modifierText).toBe('(120秒窗口)');
    });

    it('should render preview values', () => {
        const context: RenderContext = { isPreview: true };
        expect(widget.render(createItem('total-speed'), context, DEFAULT_SETTINGS)).toBe('合计: 127.7 t/s');
    ...
    it('should compute total speed from selected window metrics', () => {
        ...
        expect(widget.render(item, context, DEFAULT_SETTINGS)).toBe('合计: 150.0 t/s');
    });
