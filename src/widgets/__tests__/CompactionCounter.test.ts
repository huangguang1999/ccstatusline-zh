...
describe('CompactionCounterWidget', () => {
    describe('metadata', () => {
        it('has correct display name', () => {
            expect(new CompactionCounterWidget().getDisplayName()).toBe('压缩计数');
        });
    ...
    describe('editor', () => {
        it('uses f and n as keybinds for the default format', () => {
            expect(new CompactionCounterWidget().getCustomKeybinds(ITEM)).toEqual([
                ...
                { key: 's', label: '(s)按触发方式分组', action: 'toggle-triggers' },
                { key: 't', label: '(t)显示回收 token 数', action: 'toggle-reclaimed' },
                { key: 'h', label: '(h)零时隐藏', action: 'toggle-hide-zero' },
                { key: 'g', label: '(g)符号', action: 'edit-symbol-override' }
            ]);
        });
        ...
        it('shows nerd font in the editor display when enabled', () => {
            expect(new CompactionCounterWidget().getEditorDisplay({
                ...
                displayText: '压缩计数',
                modifierText: '(icon-space-number, Nerd 字体)'
            });
        });
        ...
        it('shows hide zero in the editor display when enabled', () => {
            expect(new CompactionCounterWidget().getEditorDisplay({
                ...
                displayText: '压缩计数',
                modifierText: '(icon-space-number, 零时隐藏)'
            });
        });
        ...
        it('shows trigger split in the editor display when enabled', () => {
            expect(new CompactionCounterWidget().getEditorDisplay({
                ...
                displayText: '压缩计数',
                modifierText: '(icon-space-number, 按触发分组)'
            });
        });
        ...
        it('shows reclaimed in the editor display when enabled', () => {
            expect(new CompactionCounterWidget().getEditorDisplay({
                ...
                displayText: '压缩计数',
                modifierText: '(icon-space-number, 回收 token)'
            });
        });
    });
});
