...
describe('VoiceStatusWidget', () => {
    describe('metadata', () => {
        it('has correct display name', () => {
            expect(new VoiceStatusWidget().getDisplayName()).toBe('语音状态');
        });

        it('has correct description', () => {
            expect(new VoiceStatusWidget().getDescription()).toBe('显示 Claude Code 语音输入是否启用');
        });

        it('has correct category', () => {
            expect(new VoiceStatusWidget().getCategory()).toBe('核心');
        });
    ...
    describe('editor configuration', () => {
        ...
        it('shows the configured format and nerd font in the editor display', () => {
            expect(new VoiceStatusWidget().getEditorDisplay({
                ...
                displayText: '语音状态',
                modifierText: '(icon-text, Nerd 字体)'
            });
        });
    ...
    describe('render() - format icon-text', () => {
        ...
        it('renders 🎤 off when OFF (standard)', () => {
            vi.spyOn(claudeSettings, 'getVoiceConfig').mockReturnValue({ enabled: false });
            expect(new VoiceStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('🎤 关闭');
        });

        it('renders 🎤 on when ON (standard)', () => {
            vi.spyOn(claudeSettings, 'getVoiceConfig').mockReturnValue({ enabled: true });
            expect(new VoiceStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('🎤 启用');
        });

        it('renders mic-slash off when OFF (nerd font)', () => {
            vi.spyOn(claudeSettings, 'getVoiceConfig').mockReturnValue({ enabled: false });
            expect(new VoiceStatusWidget().render(FORMAT_NERD_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe(' 关闭');
        });

        it('renders mic on when ON (nerd font)', () => {
            vi.spyOn(claudeSettings, 'getVoiceConfig').mockReturnValue({ enabled: true });
            expect(new VoiceStatusWidget().render(FORMAT_NERD_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe(' 启用');
        });
    });
    ...
    describe('render() - format word', () => {
        ...
        it('renders "voice off" when OFF', () => {
            vi.spyOn(claudeSettings, 'getVoiceConfig').mockReturnValue({ enabled: false });
            expect(new VoiceStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('语音 关闭');
        });

        it('renders "voice on" when ON', () => {
            vi.spyOn(claudeSettings, 'getVoiceConfig').mockReturnValue({ enabled: true });
            expect(new VoiceStatusWidget().render(FORMAT_ITEM, makeContext(), DEFAULT_SETTINGS)).toBe('语音 启用');
        });
    });
    ...
    describe('render() - preview mode', () => {
        ...
        it('renders the ON state for word format with nerd font', () => {
            ...
            expect(new VoiceStatusWidget().render(item, makeContext({ isPreview: true }), DEFAULT_SETTINGS)).toBe('语音 启用');
        });
    });
