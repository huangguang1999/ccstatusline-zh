...
/**
 * 统计当前会话中上下文压缩（compaction）发生的次数。
 *
 * Claude Code 在上下文窗口接近上限时会周期性地对对话进行压缩（摘要）。
 * 该组件通过统计转录文件中的 compact_boundary 标记来追踪压缩次数。
 *
 * 默认显示 ↻ N，压缩发生前显示 ↻ 0。可配置为零时隐藏。
 */
export class CompactionCounterWidget implements Widget {
    ...
    getCustomKeybinds(item?: WidgetItem): CustomKeybind[] {
        ...
        keybinds.push({ key: 's', label: '(s)触发器分组', action: TOGGLE_TRIGGERS_ACTION });
