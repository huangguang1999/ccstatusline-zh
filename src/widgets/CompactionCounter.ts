...
const RECLAIMED_SLOT: SymbolSlot = { id: 'symbolReclaimed', label: '回收', defaultSymbol: '↓' };
...
function formatTriggerSuffix(byTrigger: CompactionData['byTrigger']): string {
    const parts: string[] = [];
    if (byTrigger.auto > 0) {
        parts.push(`${byTrigger.auto} 自动`);
    }
    if (byTrigger.manual > 0) {
        parts.push(`${byTrigger.manual} 手动`);
    }
    if (byTrigger.unknown > 0) {
        parts.push(`${byTrigger.unknown} 未知`);
    }
...
function formatCount(count: number, format: CompactionCounterFormat, icon: string): string {
    switch (format) {
        ...
        case 'text-and-number': return `压缩次数: ${count}`;
...
export class CompactionCounterWidget implements Widget {
    ...
    getDescription(): string { return '统计当前会话中上下文压缩（compaction）发生的次数'; }
    getDisplayName(): string { return '压缩计数'; }
    ...
    getEditorDisplay(item: WidgetItem): WidgetEditorDisplay {
        ...
        if (isMetadataFlagEnabled(item, SHOW_TRIGGERS_METADATA_KEY)) {
            modifiers.push('按触发分组');
        }
        if (isMetadataFlagEnabled(item, SHOW_RECLAIMED_METADATA_KEY)) {
            modifiers.push('回收 token');
        }
    ...
    getCustomKeybinds(item?: WidgetItem): CustomKeybind[] {
        const keybinds: CustomKeybind[] = [
            { key: 'f', label: '(f)格式切换', action: CYCLE_FORMAT_ACTION }
        ];
        ...
        keybinds.push({ key: 's', label: '(s)按触发方式分组', action: TOGGLE_TRIGGERS_ACTION });
        keybinds.push({ key: 't', label: '(t)显示回收 token 数', action: TOGGLE_RECLAIMED_ACTION });
