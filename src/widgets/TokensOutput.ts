...
export class TokensOutputWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        if (context.isPreview) {
            return formatRawOrLabeledValue(item, '输出: ', '3.4k');
        }

        const outputTotalTokens = getContextWindowOutputTotalTokens(context.data);
        if (outputTotalTokens !== null) {
            return formatRawOrLabeledValue(item, '输出: ', formatTokens(outputTotalTokens));
        }

        if (context.tokenMetrics) {
            return formatRawOrLabeledValue(item, '输出: ', formatTokens(context.tokenMetrics.outputTokens));
        }
        return null;
    }
