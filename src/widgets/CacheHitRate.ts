...
export class CacheHitRateWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        if (context.isPreview) {
            return formatRawOrLabeledValue(item, '缓存命中率: ', '87.0%');
        }

        const hideWhenEmpty = isCacheHideWhenEmptyEnabled(item);
        const tokens = getCacheTokens(context, isCacheSessionScope(item));
        if (!tokens) {
            return hideWhenEmpty ? null : formatRawOrLabeledValue(item, '缓存命中率: ', 'n/a');
        }

        const hitRate = getCacheHitRate(tokens);
        if (hitRate === null) {
            return hideWhenEmpty ? null : formatRawOrLabeledValue(item, '缓存命中率: ', '0.0%');
        }
        ...
        return formatRawOrLabeledValue(item, '缓存命中率: ', `${hitRate.toFixed(1)}%`);
    }
