...
export class CacheHitRateWidget implements Widget {
    ...
    getDescription(): string { return '显示提示缓存命中率（缓存读取次数 vs 缓存写入次数）'; }
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        if (context.isPreview) {
            return formatRawOrLabeledValue(item, '缓存命中: ', '87.0%');
        }

        const hideWhenEmpty = isCacheHideWhenEmptyEnabled(item);
        const tokens = getCacheTokens(context, isCacheSessionScope(item));
        if (!tokens) {
            return hideWhenEmpty ? null : formatRawOrLabeledValue(item, '缓存命中: ', 'n/a');
        }

        const hitRate = getCacheHitRate(tokens);
        if (hitRate === null) {
            return hideWhenEmpty ? null : formatRawOrLabeledValue(item, '缓存命中: ', '0.0%');
        }
        ...
        return formatRawOrLabeledValue(item, '缓存命中: ', `${hitRate.toFixed(1)}%`);
    }
