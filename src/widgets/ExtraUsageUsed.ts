...
export class ExtraUsageUsedWidget implements Widget {
    ...
    getDescription(): string { return '显示超用量已消费金额（按量付费超出部分）'; }
    getDisplayName(): string { return '超用量已用'; }
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        if (context.isPreview) {
            return formatRawOrLabeledValue(item, '超用量已用: ', '$106.00');
        }
        ...
        if (data.extraUsageEnabled === false) {
            return isHideExtraUsageDisabledEnabled(item)
                ? null
                : formatRawOrLabeledValue(item, '超用量已用: ', 'n/a');
        }
        ...
        return formatRawOrLabeledValue(item, '超用量已用: ', formatted);
    }
