...
export class ExtraUsageUsedWidget implements Widget {
    ...
    getDescription(): string { return '显示按量付费超额使用花费的美元金额'; }
    getDisplayName(): string { return '超额使用花费'; }
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        if (context.isPreview) {
            return formatRawOrLabeledValue(item, '超额花费: ', '$106.00');
        }
        ...
        if (data.extraUsageEnabled === false) {
            return isHideExtraUsageDisabledEnabled(item)
                ? null
                : formatRawOrLabeledValue(item, '超额花费: ', 'n/a');
        }
        ...
        return formatRawOrLabeledValue(item, '超额花费: ', formatted);
    }
