...
export class FreeMemoryWidget implements Widget {
    ...
    getDescription(): string { return '显示系统内存使用情况（已用/总量）'; }
    getDisplayName(): string { return '内存用量'; }
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        if (context.isPreview) {
            return item.rawValue ? '12.4G/16.0G' : '内存: 12.4G/16.0G';
        }
        ...
        return item.rawValue ? value : `内存: ${value}`;
    }
