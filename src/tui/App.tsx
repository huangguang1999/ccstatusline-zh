...
export function buildConfigLoadWarning(configLoadError: string | null): string | null {
    ...
    return `⚠ ${configLoadError}——当前显示默认配置；在此保存将覆盖原文件。`;
}
...
export function buildInvalidConfigSaveConfirm(
    ...
    return {
        message: `${configLoadError}，原文件仍保留在磁盘上。保存将用当前配置替换原文件，是否继续？`,
        action: () => {
            onConfirm();
...
export const App: React.FC = () => {
    ...
    const handleMainMenuSelect = async (value: MainMenuOption) => {
        switch (value) {
            ...
            case 'starGithub':
                setConfirmDialog({
                    message: `在浏览器中打开 ccstatusline-zh GitHub 仓库？\n\n${GITHUB_REPO_URL}`,
                    action: () => {
                        const result = openExternalUrl(GITHUB_REPO_URL);
                        if (result.success) {
                            setFlashMessage({
                                text: '✓ 已在浏览器中打开 GitHub 仓库',
                                color: 'green'
                            });
                        } else {
                            setFlashMessage({
                                text: `✗ 无法打开浏览器。请访问: ${GITHUB_REPO_URL}`,
                                color: 'red'
                            });
                        }
                        setScreen('main');
