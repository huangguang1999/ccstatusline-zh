...
export function normalizePickerState(
    ...
    const effectiveCategory = hasTopLevelSearch ? '全部' : (selectedCategory ?? '全部');
...
function getPickerViewState(
    ...
    const filteredWidgets = filterWidgetCatalog(widgetCatalog, selectedCategory ?? '全部', widgetPicker.widgetQuery);
