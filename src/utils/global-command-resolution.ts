...
function isTransientBunxStatusLinePath(filePath: string): boolean {
    ...
    return /(?:^|\/)bunx-[^/]*ccstatusline-zh@[^/]+\/node_modules\/\.bin\/ccstatusline-zh(?:\.(?:cmd|ps1))?$/i.test(normalized);
}
...
function getResolutionWarning(
    ...
    if (resolvedDirs.length > 1) {
        return `⚠ PATH 中存在多个 ccstatusline-zh 可执行文件，Claude Code 将使用第一个匹配：${firstResolvedPath}。\n其他匹配：${formatPathList(resolvedPaths.slice(1))}`;
    }

    if (expectedBinDir && !isPathInsideDir(firstResolvedPath, expectedBinDir)) {
        return `⚠ ccstatusline-zh 解析到 ${firstResolvedPath}，不在 ${packageManager} 全局 bin 目录 (${expectedBinDir}) 内，Claude Code 仍会运行 PATH 中的首个匹配。`;
    }
...
export function inspectGlobalCommandResolution(
    ...
    const resolvedPaths = getPersistentCommandResolutionPaths(getCommandResolutionPaths('ccstatusline-zh', options));
