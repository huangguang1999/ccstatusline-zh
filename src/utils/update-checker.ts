...
export const NPM_REGISTRY_LATEST_URL = 'https://registry.npmjs.org/ccstatusline-zh/latest';
...
export function fetchLatestNpmVersion(timeoutMs = DEFAULT_REGISTRY_TIMEOUT_MS): Promise<string> {
    return new Promise((resolve, reject) => {
        const request = https.request(
            NPM_REGISTRY_LATEST_URL,
            {
                headers: {
                    ...
                    'User-Agent': 'ccstatusline-zh'
                }
            },
            (response) => {
                if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) {
                    response.resume();
                    reject(new Error(`npm 仓库返回 HTTP ${response.statusCode ?? 'unknown'}`));
                    return;
                }
                ...
                response.on('end', () => {
                    try {
                        ...
                        if (typeof parsed.version !== 'string' || parsed.version.trim() === '') {
                            reject(new Error('npm 仓库响应中未包含版本号'));
                            return;
                        }
        ...
        request.setTimeout(timeoutMs, () => {
            request.destroy(new Error(`npm 仓库请求超过 ${timeoutMs}ms 未响应`));
        });
...
export function runGlobalPackageInstall(
    ...
    const args = packageManager === 'npm'
        ? ['install', '-g', `ccstatusline-zh@${version}`]
