...
function getBinaryPathCandidates(binDir: string, platform: NodeJS.Platform): string[] {
    ...
    return extensions.map(extension => appendPathSegment(binDir, `ccstatusline-zh${extension}`));
}
...
function getNpmGlobalPackageVersion(platform: NodeJS.Platform): string | null {
    try {
        ...
        return rootDir
            ? readPackageVersion(appendPathSegment(appendPathSegment(rootDir, 'ccstatusline-zh'), 'package.json'))
            : null;
    } catch {
        return null;
    }
...
function getActiveResolutionWarning(
    ...
    if (!active.resolvedPath) {
        return '⚠ 当前 PATH 中找不到 ccstatusline-zh，Claude Code 会直接调用 ccstatusline-zh，请重启终端或更新 PATH。';
    }

    const resolvedDirs = getUniqueResolvedDirs(resolvedPaths);
    if (resolvedDirs.length > 1) {
        return `⚠ PATH 中存在多个 ccstatusline-zh 可执行文件，Claude Code 将使用第一个匹配：${active.resolvedPath}。\n其他匹配：${formatPathList(resolvedPaths.slice(1))}`;
    }

    if (active.packageManager === 'unknown') {
        return `⚠ ccstatusline-zh 解析到 ${active.resolvedPath}，但不在已检测到的 npm 或 bun 全局 bin 目录内。`;
    }
...
export function inspectActiveGlobalCommand({
    ...
    const resolvedPaths = getPersistentCommandResolutionPaths(getCommandResolutionPaths('ccstatusline-zh', { platform }));
