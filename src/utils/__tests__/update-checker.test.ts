...
describe('update checker', () => {
    ...
    it('offers bun global update for PATH-resolved pinned bun installs', () => {
        ...
        expect(result.actions.map(action => action.command)).toEqual(['bun add -g ccstatusline-zh@2.3.0']);
    });
    ...
    it('uses npm.cmd for Windows global npm installs', async () => {
        ...
        expect(execFileSpy.mock.calls[0]?.[1]).toEqual(['install', '-g', 'ccstatusline-zh@2.3.0']);
    });
