...
describe('openExternalUrl', () => {
    ...
    it('uses open on macOS', () => {
        ...
        const result = openExternalUrl('https://github.com/huangguang1999/ccstatusline-zh');

        expect(result).toEqual({ success: true });
        expect(mockSpawnSync.mock.calls[0]?.[0]).toBe('open');
        expect(mockSpawnSync.mock.calls[0]?.[1]).toEqual(['https://github.com/huangguang1999/ccstatusline-zh']);
    });
    ...
    it('uses cmd start on Windows', () => {
        ...
        expect(mockSpawnSync.mock.calls[0]?.[1]).toEqual(['/c', 'start', '', 'https://github.com/huangguang1999/ccstatusline-zh']);
    });
    ...
    it('falls back to gio open when xdg-open fails', () => {
        ...
        expect(mockSpawnSync.mock.calls[1]?.[1]).toEqual(['open', 'https://github.com/huangguang1999/ccstatusline-zh']);
    });
